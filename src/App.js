...
class Game extends React.Component {
  render() {
    let width = this.sanitize(
      prompt("Please enter board width"),
      'width'
    );
    while (!width.valid) {
      width = this.sanitize(prompt(width.msg), 'width');
    }

    let height = this.sanitize(
      prompt("Please enter board height"),
      'height'
    );
    while (!height.valid) {
      height = this.sanitize(prompt(height.msg), 'height');
    }
  ...
  render() {
    return (
      <button className="square" onClick={() => this.props.onClick()}>
        <img src={this.props.element} style={{ display: this.props.display }} alt=""/>
      </button>
    );
  }
}
